...
export async function getVendorById(id: string): Promise<Vendor | null> {
  ...
  try {
    const db = getFirestoreDb();
    const snap = await getDoc(doc(db, COL_VENDORS, id));
...
export async function getRecentEntries(limit = 10): Promise<LedgerEntry[]> {
  let supabaseFail: string | null = null;
  if (isSupabaseConfigured()) {
    try {
      const sb = getSupabase();
  ...
  try {
    const db = getFirestoreDb();
    const snap = await getDocs(
      query(collection(db, COL_ENTRIES), orderBy('date', 'desc'), orderBy('created_at', 'desc'))
    );
    return snap.docs.slice(0, limit).map(d => d.data() as LedgerEntry);
  } catch (e) {
    throw new Error(
      supabaseFail
        ? `Supabase failed (${supabaseFail}) and Firebase failed (${String(e)}).`
